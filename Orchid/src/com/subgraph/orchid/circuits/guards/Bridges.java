...
public class Bridges {
	...
	private class DescriptorDownloader implements Runnable {
		...
		private void downloadDescriptor() {
			System.out.println("Feng:Downloading descriptor for bridge: "+ target);
			...
			try {
				...
				if(descriptor != null) {
					System.out.println("Feng:Descriptor received for bridge "+ target +". Adding to list of usable bridges");
					//logger.fine("Descriptor received for bridge "+ target +". Adding to list of usable bridges");
					target.setDescriptor(descriptor);
					synchronized(lock) {
						bridgeRouters.add(target);//入口router
						//System.out.println("Feng:"+target.getAverageBandwidth());
						lock.notifyAll();//通告lock
					}
				}
	...
	BridgeRouter chooseRandomBridge(Set<Router> excluded) throws InterruptedException {
		...
		synchronized(lock) {
			...
			return candidates.get(random.nextInt(candidates.size()));//随机选取
		}
	}
