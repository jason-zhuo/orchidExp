...
public class NTorCircuitExtender {
	...
	CircuitNode extendTo() {
		...
		if(finalRouterSupportsExtend2()) {//使用此模式进行extend
			//System.out.println("Feng:"+"Extend Mode:"+"Extending circuit to "+ router.getNickname() + " with NTor inside RELAY_EXTEND2");
			logger.fine("Extending circuit to "+ router.getNickname() + " with NTor inside RELAY_EXTEND2");
			return extendWithExtend2(onion);
		} else {
			//System.out.println("Feng:"+"Extend Mode:"+"Extending circuit to "+ router.getNickname() + " with NTor inside RELAY_EXTEND");
