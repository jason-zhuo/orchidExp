...
public class SocksClientTask implements Runnable {
	...
	public void run() {
		final int version = readByte();//版本号
	...
	private void dispatchRequest(int versionByte) {
		...
		case 'G':
		case 'P':
			sendHttpPage();//http传输未实现
			break;
		case 4:
			processRequest(new Socks4Request(config, socket));//v4
			break;
		case 5:
			processRequest(new Socks5Request(config, socket));//v5
			break;
		default:
	...
	private void processRequest(SocksRequest request) {
		try {
			...
			try {
				...
				System.out.println("Feng:SockClient:SOCKS CONNECT to "+ request.getTarget()+ " completed");
