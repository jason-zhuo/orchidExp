...
public class DashboardConnection implements Runnable {
	...
	public void run() {
		...
		try {
			final PrintWriter writer = new PrintWriter(socket.getOutputStream());//printer
			handle = refreshExecutor.scheduleAtFixedRate(createRefreshRunnable(writer), 0, REFRESH_INTERVAL, TimeUnit.MILLISECONDS);//handle
			runInputLoop(socket.getInputStream());//read data from internet
		} catch (IOException e) {
			closeQuietly(socket);
		} finally {
			if(handle != null) {
			...
			refreshExecutor.shutdown();//game over
		}
	}
